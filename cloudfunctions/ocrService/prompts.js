...
// 公共核心约束（编号 1-12；各模式特有约束由调用方在【公共约束】前追加）
...
14. result="fail" 仅当：OCR 文本既不是保单也不是现价表 / 完全无法识别
...
【不可变更的核心约束】
13. 顶层必须是 JSON 数组，为每张图片输出一个对象
14. 每个对象必须带 idx 声明对应的图片编号（如【图片_3】→ idx=3）；若某张图无法提取，该元素 result="fail" 且仍保留该 idx；idx 不得重复
15. 每张图独立判断 document_type 和 result，单张图失败不影响其他图
16. cash_values 中 y=保单年度（整数），v=现金价值（元，纯数字），n=可选特殊标注
