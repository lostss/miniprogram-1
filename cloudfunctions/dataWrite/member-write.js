 ...
 * member-write — 成员操作领域
 *
 * 导出：recordField / updateMember / deleteMember
 *
 * 依赖关系（单向，无循环）：
...
// ---------- deleteMember ----------
// 单成员删除：移除 members 文档 + 级联软删该成员相关事实
// P0-3：补充反向关系清理——其他成员指向被删成员的关系边（配偶/子女/父母）也需 supersede
async function deleteMember(db, openid, event) {
  ...
  // P0-3 反向：其他成员指向被删成员的关系边（object_id=mid 且 predicate 为关系类）
  await ws.silentUpdateWhere('facts', { family_id: familyId, _openid: openid, predicate: _.in(['配偶', '子女', '父母']), object_id: mid, status: 'active' }, { status: 'superseded' }).catch(e => { console.error('[dataWrite] deleteMember 反向关系 supersede 失败:', e.message); failures.push('facts_reverse') })
  await ws.triggerHooks()
...
module.exports = { recordField, updateMember, deleteMember }
